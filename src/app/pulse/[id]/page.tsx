...
export default function PulseDetailsPage() {
  ...
  const calculatePeriodStats = useCallback((trades: Trade[], range: TimeRange) => {
    const now = new Date();
    ...
    const periodStart = new Date(now.getTime() - (daysToSubtract * 24 * 60 * 60 * 1000));
    const previousPeriodStart = new Date(periodStart.getTime() - (daysToSubtract * 24 * 60 * 60 * 1000));

    // Sort trades by date
    const sortedTrades = [...trades].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    
    // Get initial trades (first period's worth of trades)
    const initialPeriodEnd = new Date(new Date(sortedTrades[0]?.date || now).getTime() + (daysToSubtract * 24 * 60 * 60 * 1000));
    const initialTrades = sortedTrades.filter(trade => new Date(trade.date) <= initialPeriodEnd);

    // Current and previous period trades
    const currentPeriodTrades = trades.filter(trade => new Date(trade.date) >= periodStart);
    ...
    setPeriodStats({
      winRate: { current: currentStats.winRate, previous: previousStats.winRate, initial: initialStats.winRate },
      totalPL: { current: currentStats.totalPL, previous: previousStats.totalPL, initial: initialStats.totalPL },
      plPercentage: { current: currentStats.plPercentage, previous: previousStats.plPercentage, initial: initialStats.plPercentage },
      trades: { current: currentStats.trades, previous: previousStats.trades, initial: initialStats.trades }
    });
  }, [pulse?.accountSize]);
  ...
  return (
    <div className="min-h-screen p-0 md:p-6 space-y-4 md:space-y-6">
      <PulseHeader
        ...
        onComparisonTypeChange={() => setComparisonType(prev => prev === 'PERIOD' ? 'START' : 'PERIOD')}
