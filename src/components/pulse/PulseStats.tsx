...
function StatChange({ current, previous, initial, comparisonType, format, isTradeCount }: StatProps) {
  ...
  const percentChange = compareValue !== 0 
    ? (diff / Math.abs(compareValue)) * 100 
    : 0;
  ...
  return (
    <div className={`absolute top-3 right-3 flex items-center space-x-1 ${color}`}>
      <span className="text-xs">
        {isEqual ? '0%' : `${format ? format(percentChange) : percentChange.toFixed(1)}%`}
      </span>
...
export default function PulseStats({ stats, comparisonType }: PulseStatsProps) {
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
      ...
      <div className="bg-dark p-3 md:p-4 rounded-lg border border-gray-800 relative">
        <h3 className="text-gray-400 text-xs md:text-sm">P/L %</h3>
        <p className="text-lg md:text-2xl font-bold text-foreground mt-1">
          {stats.plPercentage.current.toFixed(2)}%
        </p>
        {stats.plPercentage[comparisonType === 'PERIOD' ? 'previous' : 'initial'] !== 0 && (
          <StatChange 
            {...stats.plPercentage} 
            comparisonType={comparisonType}
            format={val => val.toFixed(1)}
