...
import { ChevronLeft, ChevronRight, TableIcon, CalendarIcon } from "lucide-react"
...
export default function TradeCalendar({ trades, pulse, onAddTrade, viewType, onViewTypeChange }: TradeCalendarProps) {
  ...
  const getTotalPL = (dateKey: string): number => {
    ...
    return tradesByDate[dateKey].reduce((total, trade) => total + trade.profitLoss, 0)
  }
      ...
      <div className="p-3 md:p-4 border-b border-gray-800">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-4">
            <h2 className="text-base md:text-lg font-semibold">Trade Calendar</h2>
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-7 gap-1">
          {days.map((day, i) => {
            ...
            const hasLosses = dayTrades.some(trade => trade.outcome === "Loss")
            
            let borderColor = "border-gray-800"
            if (dayTrades.length > 0) {
              borderColor = totalPL > 0 ? "border-green-600" : totalPL < 0 ? "border-red-600" : "border-gray-800"
            }
            ...
            return (
              <div
                ...
                onClick={() => dayTrades.length > 0 && setSelectedTrade(dayTrades[0])}
              >
                ...
                {dayTrades.length > 0 && (
                  <div className="mt-1">
                    <div className={`text-xs font-medium ${
                      totalPL > 0 ? "text-green-400" : totalPL < 0 ? "text-red-400" : "text-gray-300"
                    }`}>
                    ...
                    <div className="flex gap-1 mt-1">
                      {hasWins && <div className="w-2 h-2 rounded-full bg-green-500"></div>}
                      {hasLosses && <div className="w-2 h-2 rounded-full bg-red-500"></div>}
                    </div>
                  </div>
                )}
              </div>
